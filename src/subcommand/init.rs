...
impl RgitCommand for InitCommand {
    fn run(&self) -> RgitResult {
        ...
        match fs::create_dir(RGIT_DIR) {
            Ok(()) => (),
            Err(e) => return RgitResult::Fatal(format!("Could not create the repository with error ({})", e)),
        }
        ...
        for dir in SUB_DIRS {
            match fs::create_dir(dir) {
                Ok(()) => (),
                Err(e) => errors.push(format!("\nFailed to create {} subdir with error {}", dir, e)),
            }
        }

        if errors.len() > 0 {
            return RgitResult::Warning(format!("Repository created with warnings:{}", errors.join("")));
        }
